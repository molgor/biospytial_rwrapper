...
accept.all  <- model.sample$accept.all
results  <- SummariseResults(samples.Y = samples.Y, samples.fitted = samples.fitted, samples.beta = samples.beta,  samples.re = samples.re, samples.tau2 = samples.tau2, samples.sigma2 =  samples.sigma2, samples.loglike = samples.loglike, X = X, X.standardised = X.standardised, X.indicator = X.indicator, X.mean = X.mean, X.sd = X.sd, Y = Y, trials = trials, p = p, offset = offset, accept.all = accept.all,formula = formula, n.miss = n.miss)

#summary.results  <- sm$results
#mean.prob  <- sm$mean.prob
#samples.beta.orig  <- sm$samples.beta.orig
#modelfit  <-  sm$modelfit
#accept.final  <- sm$accept.final
